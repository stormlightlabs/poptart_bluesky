...
Map<String, dynamic> post({required String text, List<RichtextFacet>? facets}) {
  return FeedPostRecord(text: text, facets: facets, langs: const ['en'], createdAt: DateTime.now().toUtc()).toJson();
}
...
ListViewBasic listViewBasic({required String name}) {
  return ListViewBasic(
    uri: const AtUri('at://did:plc:fake/app.bsky.graph.list/fake'),
    cid: _kFakeCid,
    name: name,
    purpose: const ListPurpose.knownValue(data: KnownListPurpose.appBskyGraphDefsModlist),
    indexedAt: DateTime.now().toUtc(),
  );
}

Label label({required String uri, required String val, String? src}) {
  return Label(src: src ?? 'did:plc:fake-labeler', uri: uri, val: val, cts: DateTime.now().toUtc());
}
