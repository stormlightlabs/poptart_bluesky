...
final class _AllExtractor implements Extractor {
  ...
  @override
  List<Entity> execute(final BlueskyText text, [final ExtractorConfig? config]) => orderByIndicesStart([
    ...config!.handles!,
...
final class _LinksExtractor implements Extractor {
  ...
  List<Entity> execute(final BlueskyText text, [final ExtractorConfig? config]) {
    ...
    for (final match in validUrlRegex.allMatches(text.value)) {
      ...
      final int endPosition = match.end;
      ...
      if (protocol.isEmpty) {
        ...
        validAsciiDomainRegex.allMatches(domain).forEach((match) {
          ...
          final asciiStartPosition = domain.indexOf(asciiDomain, asciiEndPosition);
          asciiEndPosition = asciiStartPosition + asciiDomain.length;

          lastUrl = {'url': asciiDomain, 'start': startPosition + asciiStartPosition};
    ...
    return (config?.fromFormat ?? false)
        ? entities
        : orderByIndicesStart([...entities, ...$markdownLinks.map((e) => e.toEntity())]);
  }
  ...
  List<Entity> _getLinkEntitiesFromReplacements(final List<Replacement> replacements, final String value) =>
      replacements
          .map(
            (e) => Entity(
              type: EntityType.link,
              value: e.value,
              indices: ByteIndices(start: value.toUtf8Index(e.start), end: value.toUtf8Index(e.end)),
            ),
          )
          .toList();
  ...
  bool _isMarkdownLink(final int start, final int end, final List<MarkdownLinkEntity>? markdownLinks) {
    if (markdownLinks == null || markdownLinks.isEmpty) return false;

    for (final markdownLink in markdownLinks) {
      if (start >= markdownLink.indices.start && markdownLink.indices.end >= end) {
        return true;
      }
    }
...
final class _TagsExtractor implements Extractor {
  ...
  List<Entity> execute(final BlueskyText text, [final ExtractorConfig? config]) {
    ...
    for (final match in validHashtagRegex.allMatches(text.value)) {
      ...
      final tag = match.boundary == '#' ? '#${match.hashMark}${match.tag}' : '${match.hashMark}${match.tag}';
      ...
      entities.add(
        Entity(
          ...
          indices: ByteIndices(start: text.value.toUtf8Index(start), end: text.value.toUtf8Index(start + tag.length)),
        ),
      );
    }
