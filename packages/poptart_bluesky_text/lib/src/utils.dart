...
String getPrefixedUri(final String source) => !source.startsWith('http') ? '$httpsPrefix$source' : source;

/// Returns true if [value] contains only emoji, otherwise false.
bool isEmojiOnly(final String value) =>
    value.trim().isNotEmpty ? emojiRegex.allMatches(value).length == value.characters.length : false;
