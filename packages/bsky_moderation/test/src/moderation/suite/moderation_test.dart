import 'package:bsky_moderation/src/moderation.dart';
import 'package:bsky_moderation/src/types/behaviors/moderation_cause.dart';
...
import 'package:bsky_moderation/src/types/labels.dart';
import 'package:bsky_moderation/src/types/moderation_behavior.dart';
  ...
  group('Moderation', () {
    ...
    test('Ignores labels from unsubscribed moderators or ignored labels for '
      ...
      for (final context in ModerationBehaviorContext.values) {
        testModeration(actual: actual.getUI(context), expected: const [], context: context.name);
      }
    });
      ...
      final actual = moderateProfile(
        ...
        const ModerationOpts(
          ...
          prefs: ModerationPrefs(
            ...
            labels: {'porn': LabelPreference.ignore},
            labelers: [
              ModerationPrefsLabeler(did: 'did:web:labeler.test', labels: {'porn': LabelPreference.ignore}),
            ],
            mutedWords: [],
            hiddenPosts: [],
          ),
    ...
    test('Can manually apply hiding', () {
      final actual = moderatePost(
        ModerationSubjectPost.postView(
          data: postView(
            record: FeedPostRecord(text: 'Hello', createdAt: DateTime.now().toUtc()).toJson(),
        ...
        const ModerationOpts(
          ...
          prefs: ModerationPrefs(
            adultContentEnabled: true,
            labels: {},
            labelers: [ModerationPrefsLabeler(did: 'did:web:labeler.test', labels: {})],
            mutedWords: [],
            hiddenPosts: [],
          ),
        ),
      ...
      testModeration(
        ...
        expected: const [ModerationTestSuiteResultFlag.filter, ModerationTestSuiteResultFlag.blur],
    ...
    test('Prioritizes filters and blurs correctly on merge', () {
      ...
      expect(
        actual.getUI(ModerationBehaviorContext.contentList).filters.first.whenOrNull(label: (data) => data.label.val),
        '!hide',
      );
      expect(
        actual.getUI(ModerationBehaviorContext.contentList).filters[1].whenOrNull(label: (data) => data.label.val),
        'porn',
      );
      expect(
        actual.getUI(ModerationBehaviorContext.contentList).blurs.first.whenOrNull(label: (data) => data.label.val),
        '!hide',
      );
      expect(
        actual.getUI(ModerationBehaviorContext.contentMedia).blurs.first.whenOrNull(label: (data) => data.label.val),
        'porn',
      );
    });
    ...
    test('Prioritizes custom label definitions', () {
      final actual = moderatePost(
        ...
        ModerationOpts(
          userDid: 'did:web:alice.test',
          prefs: const ModerationPrefs(
            adultContentEnabled: true,
            labels: {'porn': LabelPreference.warn},
            labelers: [
              ModerationPrefsLabeler(did: 'did:web:labeler.test', labels: {'porn': LabelPreference.warn}),
            ],
            mutedWords: [],
            hiddenPosts: [],
          ),
    ...
    test("Doesn't allow custom behaviors to override imperative labels", () {
      ...
      testModeration(
        actual: actual.getUI(ModerationBehaviorContext.contentView),
        expected: const [ModerationTestSuiteResultFlag.blur, ModerationTestSuiteResultFlag.noOverride],
        context: ModerationBehaviorContext.contentView.name,
      );
      testModeration(
        actual: actual.getUI(ModerationBehaviorContext.contentMedia),
          ...
          prefs: const ModerationPrefs(
            ...
            labelers: [
              ModerationPrefsLabeler(
                ...
                labels: {'BadLabel': LabelPreference.hide, 'bad/label': LabelPreference.hide},
              ),
            ],
            mutedWords: [],
            hiddenPosts: [],
      ...
      final actual = moderatePost(
        ...
        ModerationOpts(
          ...
          prefs: const ModerationPrefs(
            ...
            labelers: [
              ModerationPrefsLabeler(did: 'did:web:labeler.test', labels: {'adult': LabelPreference.ignore}),
            ],
            mutedWords: [],
            hiddenPosts: [],
          ),
