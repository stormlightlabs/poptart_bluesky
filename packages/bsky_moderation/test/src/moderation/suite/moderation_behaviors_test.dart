...
const _kUsers = <String, dynamic>{
  'self': {'blocking': false, 'blockingByList': false, 'blockedBy': false, 'muted': false, 'mutedByList': false},
  'alice': {'blocking': false, 'blockingByList': false, 'blockedBy': false, 'muted': false, 'mutedByList': false},
  'bob': {'blocking': true, 'blockingByList': false, 'blockedBy': false, 'muted': false, 'mutedByList': false},
  'carla': {'blocking': false, 'blockingByList': false, 'blockedBy': true, 'muted': false, 'mutedByList': false},
  'dan': {'blocking': false, 'blockingByList': false, 'blockedBy': false, 'muted': true, 'mutedByList': false},
  'elise': {'blocking': false, 'blockingByList': false, 'blockedBy': false, 'muted': false, 'mutedByList': true},
  'fern': {'blocking': true, 'blockingByList': false, 'blockedBy': true, 'muted': false, 'mutedByList': false},
  'georgia': {'blocking': false, 'blockingByList': true, 'blockedBy': false, 'muted': false, 'mutedByList': false},
};
...
final suite = ModerationBehaviorSuiteRunner(
  users: _kUsers.map((key, value) => MapEntry(key, ModerationTestSuiteUser.fromJson(value))),
  configurations: _kConfigurations.map((key, value) => MapEntry(key, ModerationTestSuiteConfiguration.fromJson(value))),
  scenarios: _kScenarios.map((key, value) => MapEntry(key, ModerationTestSuiteScenario.fromJson(value))),
);

void main() {
  final scenarios = suite.scenarios.entries.where((entry) => !entry.key.startsWith('//')).toList();

  for (final scenario in scenarios) {
    test(scenario.key, () {
      final actual = scenario.value.subject == 'profile'
          ? moderateProfile(suite.profileScenario(scenario.value), suite.getModerationOpts(scenario.value))
          : moderatePost(suite.getPostScenario(scenario.value), suite.getModerationOpts(scenario.value));
