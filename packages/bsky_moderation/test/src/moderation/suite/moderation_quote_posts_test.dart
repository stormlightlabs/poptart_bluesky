...
import 'package:bsky_moderation/src/types/interpreted_label_value_definition.dart';
...
void main() {
  test('Moderation: custom labels', () {
    final scenarios = _kScenario
        .expand(
          (e) => [
            {'blurs': e['blurs'], 'severity': e['severity'], 'target': 'post', 'expected': e['post']},
            {'blurs': e['blurs'], 'severity': e['severity'], 'target': 'profile', 'expected': e['profile']},
            {'blurs': e['blurs'], 'severity': e['severity'], 'target': 'account', 'expected': e['account']},
          ],
        )
        .toList();
    ...
    for (final scenario in scenarios) {
      ...
      final List<Label> postLabels = [];
      final List<Label> profileLabels = [];

      if (target == 'post') {
        postLabels.add(
          m.label(uri: 'at://did:web:carla.test/app.bsky.feed.post/fake', val: 'custom', src: 'did:web:labeler.test'),
        );
        ...
        profileLabels.add(m.label(uri: 'did:web:carla.test', val: 'custom', src: 'did:web:labeler.test'));
      }
      ...
      final post = m.postView(
        ...
        embed: m.embedRecordView(
          ...
          labels: postLabels,
          author: m.profileViewBasic(handle: 'carla.test', displayName: 'Carla', labels: profileLabels),
        ),
        author: m.profileViewBasic(handle: 'bob.test', displayName: 'Bob'),
      );

      final actual = moderatePost(ModerationSubjectPost.postView(data: post), modOpts(blurs, severity));
...
List<ModerationTestSuiteResultFlag> _getExpectedResultFlags(final List<String>? flags) {
  return flags != null ? flags.map((e) => ModerationTestSuiteResultFlag.valueOf(e)).toList() : const [];
}
...
ModerationOpts modOpts(final String blurs, final String severity) {
  return ModerationOpts(
    ...
    prefs: const ModerationPrefs(
      ...
      labels: {},
      labelers: [
        ModerationPrefsLabeler(did: 'did:web:labeler.test', labels: {'custom': LabelPreference.hide}),
      ],
      mutedWords: [],
      hiddenPosts: [],
    ),
...
InterpretedLabelValueDefinition makeCustomLabel(final String blurs, final String severity) {
